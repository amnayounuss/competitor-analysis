...
export async function runJob(job: Job): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      await log('warn', `Stage C failed, continuing without competitors: ${err.message}`);
    }

    let rawPlaces = [...target, ...competitors];
    if (rawPlaces.length === 0) {
      throw new Error('No data collected — both target and competitors returned zero branches. Check brand names and refresh token.');
    }
