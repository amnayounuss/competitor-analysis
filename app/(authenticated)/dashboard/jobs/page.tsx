    ...
    <div className="p-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Review your past reports and active tracking jobs.</p>
        </div>
      ...
      <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/30 overflow-hidden">
        {(!jobs || jobs.length === 0) ? (
          <div className="py-20 text-center">
            <p className="text-slate-400 font-bold">No analysis history found.</p>
            <Link href="/dashboard/new" className="text-indigo-600 font-bold mt-2 inline-block hover:underline">Create your first analysis →</Link>
          </div>
        ) : (
                ...
                <tr key={j.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-8 py-6">
                    <Link href={`/jobs/${j.id}`} className="block">
                      <p className="font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">{j.target_name}</p>
                      <p className="text-xs text-slate-400 italic">vs {j.competitors.join(', ')}</p>
                    </Link>
                  </td>
