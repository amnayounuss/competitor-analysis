...
export default function SchedulesList() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 space-y-4">
        ...
        <p className="text-sm font-bold text-slate-400 uppercase tracking-widest"><BiInline en="Accessing Automation Registry…" /></p>
      </div>
    );
  }
  ...
  if (items.length === 0) {
    return (
      <div className="modern-card p-16 text-center" dir={isAr ? 'rtl' : 'ltr'}>
        ...
        <h3 className="text-xl font-bold text-slate-900 mb-2 tracking-tight"><BiInline en="No active schedules" /></h3>
        <p className="text-slate-500 max-w-sm mx-auto leading-relaxed">
          <BiInline en='Enable the "Run monthly" toggle when initiating a new analysis to see your recurring automations here.' />
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="grid gap-6" dir={isAr ? 'rtl' : 'ltr'}>
      {items.map(s => (
        <div key={s.id} className="modern-card p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 group hover:border-indigo-200 transition-all">
          <div className="min-w-0 flex-1 space-y-3">
            <div className="flex items-center gap-3 flex-wrap">
              <h3 className="text-lg font-bold text-slate-900 truncate tracking-tight"><BiInline en={s.target_name} /></h3>
              <span className="text-slate-400 font-medium"><BiInline en="vs" /></span>
              <div className="text-sm font-semibold text-slate-600 truncate max-w-[200px] flex items-center gap-1">
                {s.competitors.map((c, idx) => (
                  <span key={idx} className="flex items-center">
                    {idx > 0 && <span className="me-1">,</span>}
                    <BiInline en={c} />
                  </span>
                ))}
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex items-center gap-2 text-[11px] font-bold text-slate-400 uppercase tracking-wide">
                ...
                <BiInline en="Next Trigger:" /> {new Date(s.next_run_at).toLocaleDateString(lang === 'ar' ? 'ar-EG' : 'en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: 'numeric' })}
              </div>
              <div className="flex items-center gap-2 text-[11px] font-bold text-slate-400 uppercase tracking-wide">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                <BiInline en="Recipient:" /> {s.email_to}
              </div>
            </div>

            {s.last_run_at && (
              <p className="text-[10px] font-medium text-slate-400/80 italic">
                <BiInline en="Execution history: Last cycle completed on" /> {new Date(s.last_run_at).toLocaleDateString(lang === 'ar' ? 'ar-EG' : 'en-US')}
              </p>
            )}
          </div>
